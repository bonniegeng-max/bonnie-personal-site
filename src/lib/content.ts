...
export const LEARNING: LearningItem[] = [
  {
    ...
    status: '9/20 开工 · 基础期：向量建库已跑通，单元 1–7 已过一遍（链路 / Embedding / 检索 / 阈值 / Prompt 注入 / 引用绑定），卡点清零，间隔复习跑起来了，节前交初版 Demo',
  },
  {
    what: '按三阶段计划推进中',
    why: '理论懂了、实践没结合是老毛病，这次用「边学边做」治：基础期搭通 Demo → 强化期补优化版和指标 → 冲刺期出数据、录屏、定稿',
    status: '基础期进行中（9/20–9/30）· 10/17 客观题 · 10/21 全套提交',
  },
];
...
export const METHOD_CASES: MethodCase[] = [
  ...
  },
  {
    ...
    body: '现象：一批伪装成普通用户的账号在批量发布低质内容。它们不是孤立违规——背后是有组织地批量起号、统一供给内容。\n\n• 定性：设备/IP 关联这类硬手段只能覆盖一小波职业黑产；更大的主体混在正常用户里，破案钥匙是聚集性\n• 视角切换：单点打不干净——打掉一个号，相似的一批还在发。把「对内容的理解」翻译成机器可执行的判据，按内容聚集的维度去识别和处置，再对账号分层处理\n• 难点：合规创作（如影视二创）和违规群控在内容层面高度相似——内容相似 ≠ 动机相同。打击力度与误伤风险之间的分寸，才是这类治理真正的功夫所在\n• 推进阻力：人的语感翻译成机器语言会失真——人看得出「这批画风、这类型不对劲」，落到机器可执行的判据，只剩「相似度」一个旋钮，干预手段单一。阈值收紧，准上来、召掉下去，分寸要靠人来回校\n• 评估失锚：更深的坎是「漏了多少」测不准。过去的评估有独立于识别能力的锚点，而这个场景没有上帝视角——评估依赖的还是同一套聚集识别能力。识别工具自证清白，才是这类治理真正的天花板\n• 边界：「组织性」的认定是最难的部分，项目后期争议加剧，最终由新的治理形态接棒\n• 复盘·重做：既定事实改不了，但值得用现在的认知重演一遍——立项顺序反着来：先圈出合规创作的高危混淆区当保护带，再谈召回；相似度降级为复核路由，不独自定处置；最根本的是换目标锚点——治理盯「大盘违规水位」（抽样可测），不盯「识别召回率」（测不准）。漏网不再是谁的原罪，水位守住才算赢',
