...
export const METHOD_CASES: MethodCase[] = [
  {
    era: '增长期 · 成熟业务的机制迭代',
    title: '用户不来的原因，藏在一句抱怨里',
    body: '一款 C 端出行产品增长乏力，所有人都在调补贴。翻调研发现，用户不用的主因是「价格不确定性」——不知道最终会被匹配到哪、付多少。于是不砸钱，改机制：用匹配成功率做杠杆重新分配价格，先在小城市低成本验证，数据显著后推动全国全量，单量级显著提升。项目周期近一年，跨产研、数据多线协同。',
    takeaway: '数据告诉你现状在哪，人对「用户为什么不来」的判断决定增量在哪。',
  },
  {
    era: '治理期 · 大模型接入内容治理链路',
    title: '把「人的判断」翻译成「机器的规则」，再用大模型重做一遍',
    body: '内容生态面对的是海量内容与人工覆盖的天花板。核心动作是翻译：把运营对内容的理解拆成机器能执行的链路——描述场景 → 特征组合 → 识别策略。最近一步是把大模型接进这条链路：大模型管语义理解、小模型管精确执行，处置覆盖率从不足 1% 提到 85% 量级，机器批量识别替代了 20+ 人力的重复劳动。随后又把治理流程本身用 AI 重做——抽样、评估、归档串成自动化，周均策略产出提升五成。',
